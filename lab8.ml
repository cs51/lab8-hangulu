...
let firer = List.iter (WEvent.fire_event newswire) [h1; h2; h3]
...
let remo = List.iter (WEvent.remove_listener newswire) [fnn_id; buzz_id]
...
let receive_report (hline : string) =
  ignore (WEvent.add_listener publish (fun () -> (fakeNewsNetwork hline)));
  ignore (WEvent.add_listener publish (fun () -> (buzzFake hline))) ;;
...
let firer2 = List.iter receive_report [h4; h5; h6] ;;
